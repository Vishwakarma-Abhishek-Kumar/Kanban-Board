...
function Dashboard() {
	...
	const [members, setMembers] = useState([{ name: 'Abhishek Kumar', github: 'https://github.com/Vishwakarma-Abhishek-Kumar' }]);
		...
		return (
			<div>
				<div style={{ display: 'flex', marginLeft: '0.4rem' }}>
					<span onClick={() => setShowImageList(!showImageList)} className={styles.theme}>
						Change Theme
					</span>
				</div>
				{showImageList && (
					<div>
						<ul className={styles.sideImg}>
							{backgroundImages.map((image, index) => (
								<li key={index} onClick={() => changeBackground(image)} className={styles.listItem}>
									<img src={image} alt={`Background ${index + 1}`} className={styles.imagePreview} width='40' height='40' border='1px solid white' />
				...
				<div className={styles.sidemenu}>
					<ul>
						<li className={styles.listItem}>
							...
							<span className={styles.text} onClick={clearBoard}>
								Clear Board
							</span>
						</li>
